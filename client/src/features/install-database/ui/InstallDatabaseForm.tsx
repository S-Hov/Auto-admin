...
import {
    ...
    type InstallDatabaseFormValues,
} from "../model/installDatabase.schema";
...
const InstallDatabaseForm = () => {
    ...
    const onSubmit = async (data: InstallDatabaseFormValues) => {
        try {
            ...
            const connectionPromise = installDatabase.checkTheConnection(
                { ...dbPayload, type: "mysql" },
                install_token,
            );
                    ...
                    success: (response) => {
                        if (response.success) {
                            sessionStorage.setItem(
                                STORAGE_KEYS.INSTALL_TOKEN,
                                install_token,
                            );
                            return apiMessage(response);
                        }
                        throw new Error(
                            "Сервер отклонил параметры подключения",
                        );
                    },
