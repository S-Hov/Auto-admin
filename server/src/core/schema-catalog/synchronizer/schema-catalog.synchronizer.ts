...
import { markIndexesMissing, replaceIndexParts, upsertPresentIndexes } from "../repository/index-write.repository";
import type {
...
import type { DBSnapshot, SchemaScanChangeCounts, StoredField, StoredResource } from "../types/schema-catalog.types";
...
const identityKey = (parentId: number, name: string): string => `${parentId}\u0000${name}`;

const buildPresentResourceMap = (resources: StoredResource[]): Map<string, StoredResource> => {
    return new Map(
        resources
...
const buildPresentFieldMap = (fields: StoredField[]): Map<number, Map<string, StoredField>> => {
    const fieldsByResourceId = new Map<number, Map<string, StoredField>>();

    for (const field of fields) {
        if (field.state !== 'present') continue;
        const resourceFields = fieldsByResourceId.get(field.resourceId) ?? new Map<string, StoredField>();
...
const requireResource = (resources: Map<string, StoredResource>, tableName: string): StoredResource => {
...
const requireField = (
    ...
    if (!field) throw new Error(`Field ${resourceId}.${columnName} was not saved`);
    return field;
};
...
const synchronizeConstraints = async (
    ...
    const oldConstraints = await readStoredConstraints(connection, snapshot.schemaName);
    const snapshotConstraints = flattenSnapshotConstraints(snapshot.tables);
    const writeItems: ConstraintWriteItem[] = snapshotConstraints.map((constraint) => {
        const resource = requireResource(resources, constraint.tableName);
        const referencedResource = constraint.referencedSchemaName === snapshot.schemaName
    ...
    const presentKeys = new Set(writeItems.map((item) => identityKey(item.resourceId, item.constraintName)));
    ...
    await markConstraintsMissing(
        ...
        oldConstraints
            .filter((constraint) => constraint.state === 'present'
                && !presentKeys.has(identityKey(constraint.resourceId, constraint.constraintName)))
            .map((constraint) => constraint.id),
    );

    const storedConstraints = await readStoredConstraints(connection, snapshot.schemaName);
    const storedByKey = new Map(
        storedConstraints
            .filter((constraint) => constraint.state === 'present')
            .map((constraint) => [identityKey(constraint.resourceId, constraint.constraintName), constraint]),
    );
    ...
    for (const constraint of snapshotConstraints) {
        const resource = requireResource(resources, constraint.tableName);
        const stored = storedByKey.get(identityKey(resource.id, constraint.constraintName));
        if (!stored) throw new Error(`Constraint ${constraint.tableName}.${constraint.constraintName} was not saved`);
        currentConstraintIds.push(stored.id);

        for (const field of constraint.fields) {
            const localField = requireField(fields, resource.id, field.columnName);
            const referencedField = resolveReferencedField(snapshot, constraint, field.referencedColumnName, resources, fields);
    ...
    await replaceConstraintFields(connection, currentConstraintIds, constraintFieldItems);
};
...
const synchronizeIndexes = async (
    ...
    const writeItems: IndexWriteItem[] = snapshot.tables.flatMap((table) => {
        ...
        return table.indexes.map((index) => ({ resourceId: resource.id, index }));
    });
    const presentKeys = new Set(writeItems.map((item) => identityKey(item.resourceId, item.index.name)));
    ...
    const storedIndexes = await readStoredIndexes(connection, snapshot.schemaName);
    const storedByKey = new Map(
        storedIndexes
            .filter((index) => index.state === 'present')
    ...
    for (const table of snapshot.tables) {
        ...
        for (const index of table.indexes) {
            const stored = storedByKey.get(identityKey(resource.id, index.name));
            if (!stored) throw new Error(`Index ${table.name}.${index.name} was not saved`);
            ...
            for (const part of index.parts) {
                partItems.push({
                    ...
                    fieldId: part.kind === 'column'
                        ? requireField(fields, resource.id, part.columnName).id
                        : null,
...
export const synchronizeSchemaCatalog = async (
    ...
    const resourcesAfterUpsert = await readStoredResources(connection, snapshot.schemaName);
    const resources = buildPresentResourceMap(resourcesAfterUpsert);
    const fieldWriteItems: FieldWriteItem[] = snapshot.tables.flatMap((table) => {
        const resource = requireResource(resources, table.name);
        return table.columns.map((column) => ({ resourceId: resource.id, column }));
    });
    ...
    const fieldsAfterUpsert = await readStoredFields(connection, snapshot.schemaName);
    const fields = buildPresentFieldMap(fieldsAfterUpsert);
    await synchronizeConstraints(connection, snapshot, scanId, resources, fields);
